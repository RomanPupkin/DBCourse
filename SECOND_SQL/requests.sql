...
WITH ALL_MAN AS (
    ...
    WHERE SALE_DATE BETWEEN TO_DATE('2013-10-01', 'YYYY-MM-DD') AND TO_DATE('2014-12-31', 'YYYY-MM-DD')
     ...
     ALL_MONTHS AS (
         SELECT DISTINCT TRUNC(MONTHS_BETWEEN(SALE_DATE, '01-01-2014')) + 1 AS MAN_MONTH
         FROM V_FACT_SALE
         WHERE SALE_DATE BETWEEN TO_DATE('2013-10-01', 'YYYY-MM-DD') AND TO_DATE('2014-12-31', 'YYYY-MM-DD')
     ),
     ALL_MAN_MOTHS AS (
     ...
     FULL_DATA AS (
         SELECT A.MANAGER_ID,
                ...
                SALE_DATE,
                SALE_AMOUNT
         FROM ALL_MAN_MOTHS A
                  LEFT OUTER JOIN V_FACT_SALE V ON
             (MAN_MONTH = DECODE(
                         TO_NUMBER(TO_CHAR(SALE_DATE, 'YYYY')) - 2014,
                         -1,
                         TRUNC(MONTHS_BETWEEN(SALE_DATE, '01-01-2014')) + 0,
                         TRUNC(MONTHS_BETWEEN(SALE_DATE, '01-01-2014')) + 1)
                 AND A.MANAGER_ID = V.MANAGER_ID)

     ),
     MAN_THREE_MONTH_SUM AS (
         SELECT MANAGER_ID,
                MANAGER_FIRST_NAME,
                MANAGER_LAST_NAME,
                MAN_MONTH,
                --TRUNC(MONTHS_BETWEEN(SALE_DATE, '01-01-2014')) + 1 AS MAN_MONTH,
                SUM(SALE_AMOUNT) OVER (
                    PARTITION BY MANAGER_ID
                    ORDER BY MAN_MONTH RANGE BETWEEN 3 PRECEDING
                    AND 1 PRECEDING
                    ) AS SUM_BY_THREE_MONTHS
         FROM FULL_DATA
     ),
     BON_BY_MAN_BY_MONTH AS (
         SELECT MANAGER_ID,
                MANAGER_FIRST_NAME,
                MANAGER_LAST_NAME,
                MAN_MONTH,
                0.05 * MAX(SUM_BY_THREE_MONTHS) AS BONUS
     ...
     MAX_BON_BY_MONTH AS (
         SELECT DISTINCT MANAGER_ID,
                         ...
                         BONUS,
                         MAX(BONUS) OVER (
                             PARTITION BY MAN_MONTH) AS MAX_BONUS
         FROM BON_BY_MAN_BY_MONTH
         WHERE MAN_MONTH BETWEEN 1 AND 12
     )
SELECT *
FROM MAX_BON_BY_MONTH
WHERE MAX_BONUS = BONUS;
...
SELECT DISTINCT COUNTRY,
                ...
                LISTAGG(LAST_FIRST_NAME, ', ') WITHIN GROUP (ORDER BY RANK_BY_COUNTRY) OVER (PARTITION BY COUNTRY) AS LIST
                --RANK_BY_COUNTRY,
                --MANAGER_ID
